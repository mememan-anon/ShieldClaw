...
class SecureExecutionPipeline {
  constructor(options = {}) {
    ...
    // Components
    this.executor = new IsolatedExecutor({
  ...
  async executeSecure(skillName, skillCode, userInput = '', options = {}) {
    ...
    try {
      ...
      const verification = await this.verifier.verifySkill(skillName, skillCode, context);
      ...
      this.logger.info(`Stage 4: Executing in isolation`);
      
      const executionResult = await this.executor.execute(skillName, skillCode, {
        env: [
          'SHIELDCLAW_SANITIZED_INPUT=' + encodeURIComponent(sanitization.sanitized)
        ]
      });
...
async function runDemo() {
  ...
  console.log('  4. Isolated container execution');
